...
\documentclass[withtitlethanks]{actacyb}
...
% Avoid modifying page size and layout!

% Defining and renewing autoref names of some elements.
...
\hypersetup{
    ...
    pdfauthor={First Author, Second Author, and Third Author},     % author
...
\author{
First Author\thanks{Same affiliation of first and second author} \thanks{\email{firstauth@email.address}, \orcid{xxxx-xxxx-xxxx-xxxx}} , Second Author\thanksmark{2} \thanks{\email{secondauth@email.address}, \orcid{xxxx-xxxx-xxxx-xxxx}} ,
\and Third Author\thanks{Affiliation of third author (if different from the first two)} \thanks{\email{thirdauth@email.adress}, \orcid{xxxx-xxxx-xxxx-xxxx}}}
...
The paper should be written in standard \LaTeX using the provided \verb|actacyb.cls| class file. Please check for the latest version of the class file at the author kit GitHub page (\url{https://github.com/attila10x/actacyb_author_kit}).

Please do not use strange formats or non-standard packages. Avoid commands that would reconfigure the size of the page, the size and type of letters or the baselineskip. 
...
By default, Acta Cybernetica uses the \verb|hyperref| package, making it easy to create links to various targets within the document. Hyperref displays these links in blue. These links are clickable and will navigate you to the target, either within or outside the document.

You can define a URL using the \verb|\url| command. URLs are usually given in footnotes. For example:

\begin{verbatim}
The Acta Cybernetica homepage\footnote{
\url{https://cyber.bibl.u-szeged.hu/index.php/actcybern/index}}
is the starting point for authors.
\end{verbatim}
...
\medskip
Various elements in the document, such as figures, tables, sections, can be labelled using the \verb|\label| command. For example, \autoref{sec:theorems} is defined as follows:

\begin{verbatim}
\section{Theorems and Lemmas}\label{sec:theorems}
\end{verbatim}

These labels can be refernced from the text using either the commands \verb|\ref| or \verb|\autoref|. The former provides only the number of the element, while the latter also inserts the element's type as a string. For example:

\begin{verbatim}
You can read more about how to reference theorems and lemmas
in \autoref{sec:theorems}.
The \verb|\ref| command only provides the defined number,
in this case \ref{sec:theorems}.
\end{verbatim}

You can read more about how to reference theorems and lemmas in \autoref{sec:theorems}.
The \verb|\ref| command only provides the defined number, in this case \ref{sec:theorems}.

\medskip
You will find more examples of referencing in the following sections.

\section{Pictures and figures}\label{sec:figures}
Every figure should appear in the \verb|figure| environment, i.e.\ must be defined between \verb|\begin{figure}| and \verb|\end{figure}|. You should provide where you prefer to put your figure \verb|[htbp]| (h -- here, t -- top, b -- bottom, p -- page), e.g., \verb|\begin{figure}[!ht]| (insert figure here or top of next page if does not fit).

You can control the size of the included image in several ways as an option to \verb|\includegraphics|. \verb|scale| option defines the scaling factor of the image. \verb|width| specifies the width of the image which can be combined with the value of \verb|\linewidth|.
...
Before \verb|\end{figure}| give a title to your figure within the \verb|\caption{...}| command. To be able to refer to this figure later, label it with a unique identifier using the  \verb|\label{uniquelabel}| command. In the text you can refer to it with \verb|\autoref{uniquelabel}|.
Let us consider the following example:

\begin{verbatim}
An example can be seen in \autoref{fig:example}.
\begin{figure}[!ht]
\begin{center}
...
Using drawing packages such as {\verb|tikz|} or calling shell executables such as \linebreak {\verb|pygmentize|} may cause incompatibilities. Instead, generate separate PDF files of these figures and include these in the manuscript.

The \verb|standalone_tikz_to_pdf_example.tex| source file shows how to generate such a PDF that can easily be included in the manuscript (\autoref{fig:standalone_tikz_pdf_example}).

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=.75\linewidth]{standalone_tikz_to_pdf_example.pdf}
\end{center}
\caption{Standalone PDF generated separately using \texttt{tikz}\label{fig:standalone_tikz_pdf_example}}
\end{figure}
...
\end{center}
\end{table}
\autoref{tab:res} shows an example table.
\end{verbatim}
...
\begin{verbatim}
The function $\Phi$ is defined in \autoref{eq:phi}.
...
A general Interval Branch and Bound algorithm is shown in \autoref{alg:ibb}. One of the following selection rules is applied in \hyperref[step:selrule]{Step~\ref{step:selrule}}.
%Step~\ref{step:selrule}.
...
The \verb|actaabbrv.bst| file defines the style descriptions that can be used like this:

\begin{verbatim}
\bibliographystyle{actaabbrv}
...
Please look up and provide the DOI whenever possible! If not available, provide an URL or ISBN.
\verb|BibTeX| generates clickable DOI, URL and ISBN links. Check that they work correctly!
